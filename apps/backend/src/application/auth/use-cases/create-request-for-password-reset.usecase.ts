 ...
 * It finds the users by their username, creates a password reset token, and sends an email with the reset link.
 */
@Injectable()
export class CreateRequestForPasswordResetUseCase {
  private readonly logger = new Logger(
    CreateRequestForPasswordResetUseCase.name,
  );

  /**
   * @param {UsersService} userService - The users service to find users.
  ...
  async execute(params: ForgetPassword): Promise<void> {
    this.logger.log(
      `Entering CreateRequestForPasswordReset for ${params.userName}`,
    );
    const user = await this.userService.findByUserName(params.userName);
    if (user) {
      this.logger.log(`Found user for ${params.userName}`);
      ...
      this.logger.log(
        `Exit CreateRequestForPasswordReset for ${params.userName}`,
      );
    }
  }
}
